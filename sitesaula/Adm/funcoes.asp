...
function idade(datanascimento)
	...
	mes_niver=month(datanascimento)'Extrai m�s do nascimento
	dif_ano=cint(ano_atual)-cint(ano_niver)'Faz a diferen�a dos anos
	dif_mes=cint(mes_atual)-cint(mes_niver)'Faz a diferen�a dos meses
	'Verifica se a diferen�a dos meses � negativa, 
        ...
        For i = LBound(str) to UBound(str)
            cont = Len(str(i))
                If ( cont = 1 ) Then
                    ...
                    End If			
                ' Se caso usar abrevia��o EX adriano r souza Retorna R.
                ElseIf ( cont = 2 ) Then 
                    termo = termo & LCase(str(i)) & " "
                ElseIf ( cont > 2 ) Then
